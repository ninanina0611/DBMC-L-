...
- **已开发模块**: 数据序列化/反序列化；数据库管理（建库/删库/选库、建表/删表、表结构修改支持）；数据操作模块（`DataManager`：记录存储与基础 CRUD）；约束校验模块（主键唯一/非空/类型校验）；SQL 解析层（`SQLLexer` + `SQLParser`）；简易 SQL 引擎（`SQLEngine`：支持 `INSERT`/`SELECT`/`UPDATE`/`DELETE`）；Qt6 图形界面（数据库浏览器、SQL编辑器、数据视图、字段管理）；操作日志模块（`LogWidget`：四级日志、SELECT可点击重新执行、日志持久化存储）。
...
- **DDL 支持**: 支持基础 DDL：`CREATE DATABASE`、`DROP DATABASE`、`USE`、`CREATE/DROP/RENAME/ALTER TABLE ADD/DROP/MODIFY/RENAME COLUMN`；已在 `Test` 中提供对应示例。
